...
| 冒泡排序       | 相邻两数交换           | O(n^2)           | 简单逻辑、易于实现        | 每次将最大/最小“冒”到顶      |
| 选择排序       | 每轮选最小与当前位置交换 | O(n^2)           | 稳定、逻辑清晰           | 每轮选出最小/最大值          |
| 插入排序       | 每次插入到前面有序区    | O(n^2)           | 小数据、基本有序时        | 类似扑克牌插牌过程           |
...
### 冒泡排序
一种简单的排序算法，它重复走访要排序的数列，一次比较两个元素，如果它们的顺序错误，就交换它们的位置。
**走访数列的次数为数列的长度减 1**。
